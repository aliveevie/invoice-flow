...
// Transfer Parameters
const DESTINATION_ADDRESS = "0x7A768a4fF560FCFCBf21689570A79A215B5f80A8"; // Address to receive minted tokens on destination chain
...
// Public clients for waiting on transaction confirmations
...
async function approveUSDC() {
  ...
  // Ensure approval is mined before proceeding so gas estimation for burn does not revert
...
async function burnUSDC() {
  ...
  // Optional: wait for burn confirmation to improve attestation availability
  await sepoliaPublicClient.waitForTransactionReceipt({ hash: burnTx });
  return burnTx;
}
...
async function mintUSDC(attestation) {
  ...
  await avalanchePublicClient.waitForTransactionReceipt({ hash: mintTx });
}
